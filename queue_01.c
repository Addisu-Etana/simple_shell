...
/**
 * list_length -The function that determines length of linked list
...
/**
 * print_list_elements -The function that prints all elements
...
list_t *node_starts_with(list_t *node, char *prefix, char c)
{
	...
	while (node)
	{
		p = starts_with_address(node->str, prefix);
		if (p && ((c == -1) || (*p == c)))
			return (node);
		node = node->next;
	}
